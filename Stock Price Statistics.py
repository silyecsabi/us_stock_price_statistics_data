...
def check_week_max(index, friday = False):
...
def check_week_min(index, friday = False):
    # collect all prices from the week
    week_prices = []
...
for item in df.index:
    for index in df.index:
        if index.year == item.year and index.month == item.month and index.day == item.day:
            if item.dayofweek == 4:
                weekly_max.append(check_week_max(index, friday = True))
            else:
...
for item in df.index:
    for index in df.index:
        if index.year == item.year and index.month == item.month and index.day == item.day:
            if item.dayofweek == 4:
                weekly_min.append(check_week_min(index, friday = True))
            else:
...
for i in range (0, len(df.index) - 1):
    if df.index[i].month != df.index[i + 1].month:      # If true, there is a month leap
        month_prices_max.append(check_month_max(df.index[i + 1]))
    else:
...
for i in range (0, len(df.index) - 1):
    if df.index[i].month != df.index[i + 1].month:
        month_prices_min.append(check_month_min(df.index[i + 1]))
    else:
...
for i in range (0, len(df.index) - 1):
    if df.index[i].quarter != df.index[i + 1].quarter:      # there is a quarter leap
        quarter_prices_max.append(check_quarter_max(df.index[i + 1]))
    else:
...
for i in range (0, len(df.index) - 1):
    if df.index[i].quarter != df.index[i + 1].quarter:
        quarter_prices_min.append(check_quarter_min(df.index[i + 1]))
    else:
